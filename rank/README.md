...
- `rank_feature_listwise.py`: 特征工程脚本（复用 pointwise 特征 + 列表级别特征）
- `rank_lambdamart.py`: LambdaMART 模型训练和预测脚本

### Listwise 特征工程 (`rank_feature_listwise.py`)

#### 1. 复用 Pointwise 特征

复用 pointwise 方法中的所有特征，包括：

- 文章基础特征
- 用户历史点击统计特征
- 召回相关特征（ItemCF、BiNetwork、Word2Vec）
- 召回分数特征
...
- **列表规模特征**:
  - `list_size`: 列表中候选新闻的数量
  - `list_rank`: 当前文章在列表中的排名位置（基于 sim_score）

- **多样性特征**:
  - `list_category_diversity`: 列表中不同类别的数量
  - `list_category_coverage`: 类别覆盖率（不同类别数量 / 列表大小）

- **时间特征**:
  - `list_time_span`: 列表中文章创建时间的跨度（最大值 - 最小值）

- **内容特征**:
  - `list_avg_words_count`: 列表中文章的平均字数
  - `list_std_words_count`: 列表中文章字数的标准差
  - `words_count_diff_from_avg`: 当前文章字数与列表平均字数的差值

- **相似度特征**:
  - `list_avg_sim_score`: 列表中相似度分数的平均值
  - `list_std_sim_score`: 列表中相似度分数的标准差
  - `list_max_sim_score`: 列表中相似度分数的最大值
  - `list_min_sim_score`: 列表中相似度分数的最小值
  - `sim_score_diff_from_avg`: 当前文章相似度与列表平均值的差值
  - `sim_score_diff_from_max`: 当前文章相似度与列表最大值的差值

### Listwise 模型训练 (`rank_lambdamart.py`)

#### 模型配置

- **算法**: LightGBM LambdaMART (LGBMRanker)
- **目标函数**: `lambdarank` - 直接优化列表级别的排序指标
- **评估指标**: NDCG@5/10/20/40/50
- **参数**:
  - `num_leaves`: 64
  - `max_depth`: 10
  - `learning_rate`: 0.05
...
1. **数据划分**: 使用 `GroupKFold(n_splits=5)` 进行5折交叉验证，按 `user_id` 分组
2. **Group 信息**: LambdaMART 需要提供每个用户的样本数量（group），用于构造列表
3. **断点续训**:
   - 检查模型文件是否存在，如果存在则跳过训练直接加载
   - 保存验证集索引到 `fold_indices_listwise.pkl`
4. **模型训练**:
   - 使用早停机制（`early_stopping_rounds=100`）
   - 每100轮输出一次训练日志
   - 保存训练好的模型到 `user_data/model/listwise/lambdamart{fold_id}.pkl`
5. **预测与评估**:
   - 对验证集进行预测，计算 OOF (Out-of-Fold) 预测
   - 对测试集进行预测，5折模型结果平均
   - 计算评估指标：HitRate@5/10/20/40/50 和 MRR@5/10/20/40/50
6. **生成提交文件**: 使用 `gen_sub()` 函数生成最终提交结果

#### LambdaMART 原理（Listwise）

LambdaMART 结合了 LambdaRank 和 MART（梯度提升树）：

- **LambdaRank**: 通过 Lambda 梯度直接优化 NDCG 等排序指标
- **MART**: 使用梯度提升树作为基学习器
- **优势**: 直接优化列表级别的排序指标，比 pointwise 和 pairwise 方法更适合排序任务

### Listwise 使用方法

```bash
# 1. 特征工程
python rank/listwise/rank_feature_listwise.py --mode valid --logfile "test_feature_listwise.log"

# 2. 模型训练
python rank/listwise/rank_lambdamart.py --mode valid --logfile "test_lambdamart.log"

# 3. 在线预测（使用训练好的模型）
python rank/listwise/rank_lambdamart.py --mode online --logfile "test_online_listwise.log"
```

### Listwise 输出文件

- `user_data/data/{mode}/feature_listwise.pkl`: 特征文件（包含列表级别特征）
- `user_data/model/listwise/lambdamart{0-4}.pkl`: 5折模型文件
- `user_data/model/listwise/fold_indices_listwise.pkl`: 验证集索引文件（用于断点续训）
- `prediction_result/result_listwise.csv`: 最终提交文件
