...
**文章类别（category_id）**
- **业务含义**：这个特征的重要性不言而喻。就像我们平时看新闻，有人喜欢看科技新闻，有人喜欢看体育新闻，有人喜欢看娱乐八卦。用户的兴趣往往集中在某几个类别上，比如一个科技爱好者，他点击科技类文章的概率肯定比点击娱乐类文章高得多。而且我们发现，不同类别的文章受欢迎程度差异很大，有些热门类别（比如科技、财经）的文章点击率明显高于其他类别。所以知道文章属于哪个类别，对预测用户是否会点击非常重要。
- **实现方式**：和字数特征一样，我们也是从文章信息表里读取类别ID，然后合并到特征表。这样每个候选文章都带着自己的类别标签，模型就能学习到"这个用户喜欢科技类，所以科技类文章得分应该高一些"这样的规律。
...
- **业务含义**：新闻推荐和商品推荐不一样，新闻有很强的时效性。一条今天发布的新闻，肯定比一个月前发布的新闻更可能被点击。用户在看新闻的时候，潜意识里会认为"新"的新闻更有价值，因为新新闻往往包含最新的信息。所以我们用这个特征来捕捉文章的时效性，让模型知道这篇文章有多"新鲜"。不过要注意的是，不是所有用户都偏好最新新闻，有些用户可能对历史新闻也感兴趣，所以这个特征需要和其他特征配合使用。
- **实现方式**：文章信息表里存的是毫秒级的时间戳，我们把它转换成秒级的时间戳，这样计算时间差的时候更方便。然后我们把这个时间戳作为特征，模型会学习到"时间戳越大（越新）的文章，用户点击概率越高"这样的规律。
...
- **业务含义**：这个特征和ItemCF相似度有点像，但计算方式不一样。ItemCF是基于"被同一用户点击"来算相似度，而Binetwork是从网络结构的角度来看。它把用户和文章看作一个网络，如果两篇文章在网络中的位置很接近（被很多相同的用户点击），那它们就相似。这种方法能发现一些ItemCF发现不了的关联，比如一些冷门但相关的文章。所以这个特征和ItemCF是互补的，能提供额外的信息。
- **实现方式**：和ItemCF类似，我们也是在召回阶段预计算好了相似度矩阵。现在只需要查表，找到用户最后点击的文章和候选文章的Binetwork相似度。实现上也是用向量化的方法，按用户分组批量处理，提高效率。
...
2. **user_click_article_w2w_sim_sum_2**：用户最近2篇点击文章与候选文章的W2V相似度之和
   - **业务含义**：这个特征和上面的ItemCF的sum特征类似，但用的是语义相似度。我们取用户最近点击的2篇文章（为什么是2篇？因为太少了信息不够，太多了可能包含不相关的历史），分别计算它们和候选文章的语义相似度，然后求和。这样能综合反映"候选文章和用户最近阅读内容的语义匹配程度"。
   - **实现方式**：我们找到用户最近点击的2篇文章，分别计算它们的向量和候选文章向量的余弦相似度，然后加起来。实现上也是向量化的，按用户分组处理，避免重复计算向量范数。

##### 3. 用户行为特征

**用户点击文章数量（user_id_cnt）**
- **业务含义**：这个特征反映的是用户的活跃程度。活跃用户和沉默用户的点击行为模式是完全不同的。活跃用户可能每天都会看很多新闻，他们对新内容的接受度更高，也愿意尝试不同类型的文章。而沉默用户可能只是偶尔看看，他们对内容的选择会更谨慎。所以知道用户点击了多少篇文章，能帮助模型调整推荐策略，比如对活跃用户可以推荐更多样化的内容，对沉默用户则要更精准。
- **实现方式**：这个特征计算起来很简单，我们按用户ID分组，统计每个用户总共点击了多少篇文章。然后用merge操作把这个统计结果合并到特征表里，这样每个用户-文章对都能知道"这个用户总共点击过多少篇文章"。

![用户点击数量分布](assets/user_click_count_distribution.png)

**文章被点击次数（article_id_cnt）**
- **业务含义**：这个特征反映的是文章的"热度"。热门文章（被很多人点击）确实更容易被新用户点击，因为"大家都在看"本身就是一个信号。但这里有个问题，如果我们只推荐热门文章，那长尾文章（小众但可能很相关）就永远得不到曝光，这会导致推荐系统缺乏多样性。所以这个特征要和其他特征配合使用，既要利用热门文章的"群众基础"，又要避免过度推荐导致的内容同质化。
- **实现方式**：和用户点击数量类似，我们按文章ID分组，统计每篇文章被点击了多少次。然后把这个统计结果合并到特征表，让模型知道"这篇文章有多热门"。

![文章被点击次数分布](assets/article_click_count_distribution.png)

**用户-类别点击次数（user_id_category_id_cnt）**
- **业务含义**：这个特征比单纯的类别特征更精细，它反映的是"这个用户对这个类别有多喜欢"。比如一个用户点击了100篇科技类文章，但只点击了2篇娱乐类文章，那说明他明显更喜欢科技类。当我们给他推荐一篇新的科技类文章时，这个特征会告诉模型"这个用户是科技类的忠实粉丝，推荐科技类文章的成功率会很高"。这个特征能帮助模型做更精准的个性化推荐。
- **实现方式**：我们按用户ID和类别ID两个维度分组，统计每个用户对每个类别点击了多少次。比如用户A点击了10篇科技类文章，那用户A-科技类这个组合的计数就是10。然后把这个计数合并到特征表，这样当候选文章是科技类时，模型就能知道"这个用户之前看过多少篇科技类文章"。
...
2. **user_last_click_created_at_ts_diff**：候选文章创建时间与用户最后点击文章创建时间的差值
   - **业务含义**：这个特征和上面那个类似，但比较的是"文章的新鲜度"而不是"用户看到的时间"。它反映的是"候选文章和用户最近看的文章相比，哪个更新？"。如果候选文章更新，说明我们推荐的是新鲜内容；如果候选文章更旧，说明我们推荐的是历史内容。这个特征能帮助模型判断用户对内容新鲜度的偏好。
   - **实现方式**：我们找到用户最后点击的那篇文章的创建时间，然后用候选文章的创建时间减去它，得到差值。这个差值反映的是两篇文章在发布时间上的差距。
...
4. **user_click_timestamp_created_at_ts_diff_mean/std**：用户点击时间与文章创建时间的差值统计
   - **业务含义**：这个特征有两个值：均值和标准差。均值反映的是"用户通常看多新的文章"，比如均值是1小时，说明用户通常看发布1小时内的文章。标准差反映的是"用户对文章新鲜度的偏好是否稳定"，如果标准差小，说明用户总是看差不多新鲜度的文章；如果标准差大，说明用户有时候看新文章，有时候看旧文章。这两个统计值能帮助模型更全面地理解用户的时间偏好。
   - **实现方式**：我们对用户历史点击的每篇文章，计算"点击时间减去文章创建时间"（这就是用户看这篇文章时，文章已经发布了多久），然后按用户分组，计算这些差值的均值和标准差。

5. **user_click_datetime_hour_std**：用户点击时间的小时标准差
   - **业务含义**：这个特征反映的是用户阅读时间的规律性。如果标准差小，说明用户总是在固定的时间段看新闻（比如总是在早上8点和晚上8点），这样的用户可能更依赖推荐系统。如果标准差大，说明用户看新闻的时间很随机，可能是在碎片时间随便看看。这个特征能帮助模型判断用户的阅读习惯。
   - **实现方式**：我们从点击时间中提取小时数（0-23），然后按用户分组，计算这些小时数的标准差。标准差小说明用户阅读时间集中，标准差大说明阅读时间分散。
...
1. **user_clicked_article_words_count_mean**：用户历史点击文章的平均字数
   - **业务含义**：这个特征反映的是用户对文章长度的"长期偏好"。比如一个用户历史点击的文章平均都是300字，那说明他可能偏好中等长度的文章。当我们给他推荐一篇5000字的长文时，模型会知道"这篇文章比用户平时看的要长很多，可能不太合适"。这个特征能帮助模型做更精准的长度匹配。
   - **实现方式**：我们按用户分组，计算用户历史点击的所有文章的字数平均值。这个平均值反映的是"这个用户通常看多长的文章"。

![用户平均点击文章字数分布](assets/user_avg_words_count_distribution.png)

2. **user_click_last_article_words_count**：用户最后点击文章的字数
   - **业务含义**：这个特征反映的是用户的"最近偏好"。用户的兴趣可能会变化，最近看的文章长度更能反映当前偏好。比如用户之前都看短新闻，但最近开始看长文，那说明他的阅读习惯可能改变了，我们应该推荐更长的文章。这个特征和上面的平均字数特征配合使用，能同时捕捉长期偏好和短期变化。
   - **实现方式**：我们找到用户最后点击的那篇文章，读取它的字数。这个字数反映的是"用户最近看的文章有多长"。

3. **user_last_click_words_count_diff**：候选文章字数与用户最后点击文章字数的差值
   - **业务含义**：这个特征反映的是"文章长度的变化幅度"。如果差值是0，说明候选文章和用户最近看的文章长度一样；如果差值是正数，说明候选文章更长；如果差值是负数，说明候选文章更短。这个特征能帮助模型判断"用户是否能接受这个长度变化"。比如用户刚看了一篇100字的短新闻，现在推荐一篇5000字的长文，可能跨度太大，用户接受不了。
   - **实现方式**：我们用候选文章的字数减去用户最后点击文章的字数，得到差值。这个差值反映的是长度变化的幅度和方向。
