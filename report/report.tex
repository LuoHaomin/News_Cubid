...
\midrule
文章字数（words\_count） & 均值200.83，中位数196 & 反映用户阅读习惯，不同用户对文章长度偏好不同（如通勤时偏好短新闻，晚上偏好长文） \\
\midrule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{assets/words_count_distribution.png}
\caption{文章字数分布}
\end{figure}

\textbf{关键发现：}从分布图可以看出，文章字数主要集中在100-300字之间，符合新闻文章的特点。正负样本的字数分布相似，说明文章长度需要与其他特征配合使用。不同类别的文章数量差异很大，某些类别的正样本比例明显高于其他类别，说明类别特征很重要。
...
\textbf{正负样本分布：}数据分布严重不平衡，正样本（用户点击的文章）只占0.23\%，负样本占99.77\%，比例接近1:200。这种不平衡在推荐系统中很常见，因为召回阶段会给每个用户召回很多候选文章，但用户只会点击其中的一小部分。我们通过设置正样本权重（scale\_pos\_weight=10）来处理这种不平衡。
...
\begin{enumerate}
\item \textbf{LambdaRank思想}：通过定义梯度（Lambda）来直接优化NDCG等排序指标
  \begin{itemize}
  \item 对于每个样本对(i,j)，如果i的相关性高于j，则i的Lambda增加，j的Lambda减少
  \item Lambda的大小与交换位置后NDCG的提升成正比
  \end{itemize}

\item \textbf{MART实现}：使用梯度提升树（GBDT）来拟合Lambda梯度
  \begin{itemize}
...
\item \textbf{优势}：
  \begin{itemize}
  \item 直接优化NDCG等排序指标，理论上性能优于Pointwise方法
  \item 使用GBDT实现，继承了树模型的优点（特征交互、非线性建模）
  \item 训练效率高，支持大规模数据
  \end{itemize}
\end{enumerate}
...
\subsubsection{方法一：LightGBM（Pointwise）}
...
\item \textbf{理论性能分析}：
  \begin{itemize}
  \item \textbf{优点}：实现简单，训练速度快，特征重要性可解释
  \item \textbf{缺点}：无法直接优化排序指标（如NDCG），只能间接通过分类准确率优化
  \item \textbf{适用场景}：数据量大、特征维度高、需要快速迭代的场景
  \end{itemize}
\end{enumerate}
...
方面 & 理论预期 & 实际表现 & 分析 \\
\midrule
训练速度 & 快速（直方图算法） & 6.3分钟（5折） & \textcolor{green}{✅ 符合预期，训练效率高} \\
内存占用 & 低（离散化特征） & 适中 & \textcolor{green}{✅ 符合预期} \\
排序性能 & 中等（间接优化） & MRR@5=0.281 & \textcolor{orange}{⚠ 无法直接优化排序指标} \\
特征重要性 & 可解释 & 清晰（sim\_score最重要） & \textcolor{green}{✅ 符合预期} \\
过拟合风险 & 中等（需正则化） & 通过早停和正则化控制 & \textcolor{green}{✅ 控制良好} \\
\bottomrule
\end{tabular}
\caption{LightGBM理论性能 vs 实际性能}
\end{table}

\subsubsection{方法二：LambdaMART（Listwise）}
...
\item \textbf{理论性能分析}：
  \begin{itemize}
  \item \textbf{优点}：直接优化排序指标，理论上性能最优；支持列表级别特征
...
\midrule
优化目标 & 分类准确率 & NDCG等排序指标 & LambdaMART \textcolor{green}{✅} \\
样本关系 & 独立处理 & 考虑列表上下文 & LambdaMART \textcolor{green}{✅} \\
列表特征 & 难以利用 & 天然支持 & LambdaMART \textcolor{green}{✅} \\
实现复杂度 & 简单 & 复杂 & LightGBM \textcolor{green}{✅} \\
训练速度 & 快 & 中等 & LightGBM \textcolor{green}{✅} \\
可解释性 & 好 & 中等 & LightGBM \textcolor{green}{✅} \\
\bottomrule
\end{tabular}
\caption{LightGBM vs LambdaMART理论对比}
\end{table}
...
\midrule
MRR@5 & 0.281 & 0.282 & +0.001 & \textcolor{green}{✅ 符合（Listwise略优）} \\
MRR@10 & 0.298 & 0.300 & +0.002 & \textcolor{green}{✅ 符合} \\
MRR@20 & 0.306 & 0.307 & +0.001 & \textcolor{green}{✅ 符合} \\
MRR@50 & 0.309 & 0.310 & +0.001 & \textcolor{green}{✅ 符合} \\
训练时间 & 6.3分钟 & 3.2分钟 & -49\% & \textcolor{red}{❌ 意外（LambdaMART更快）} \\
\bottomrule
\end{tabular}
